...
void Menu_run(Menu *m, BG *bg) {
    ...
    Menu_draw(m);
    Menu_refresh_cursor(m);
    Menu_update_player_sprites(m, 1);
    u16 next_sound = SND_SAMPLE_CREAK_2;
    u16 tilt = 2;
    while (!m->completed) {
        random();
        Menu_input(m);
        ...
        if (!XGM_isPlayingPCM(SOUND_PCM_CH2_MSK)) {
            XGM_startPlayPCM(next_sound, 15, SOUND_PCM_CH2);
            next_sound = next_sound == SND_SAMPLE_CREAK_2 ? SND_SAMPLE_CREAK_1 : SND_SAMPLE_CREAK_2;
        }
        if (bg->theta >= 768 || bg->theta <= 256) tilt = -tilt;
        BG_tilt_by(bg, tilt);
        BG_update(bg);
...
void Menu_update_player_sprites(Menu *m, u8 n_players) {
    for (u8 i = 0; i < MAX_N_PLAYERS; ++i) {
        Sprite *sp = m->pl_sprites[i];
        if (sp) {
            SPR_releaseSprite(sp);
...
void Menu_Item_next_option(Menu_Item *mi) {
    ...
    Menu_update_player_sprites(mi->parent, mi->parent->first->option_selected + 1);
}
